...
function removeFirstOccurrences(str, value) {
  const index = str.indexOf(value); // Находим индекс первого вхождения
  return index === -1
    ? str
...
function removeLastOccurrences(str, value) {
  const index = str.lastIndexOf(value); // Находим индекс первого вхождения
  return index === -1
    ? str
    : str.slice(0, index) + str.slice(index + value.length);
}
...
function isPalindrome(str) {
  ...
  return (
    result.toUpperCase().replace(/[^A-Z]/g, '') ===
    str.toUpperCase().replace(/[^A-Z]/g, '')
  );
}
